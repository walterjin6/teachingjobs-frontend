...
import southamericapositionsData from "../utils/southamericapositions.json";
import { Helmet } from "react-helmet";
const SouthAmericaPositions = () => {  
  const dispatch = useDispatch()
  let { southamericapositions } = useParams();
  southamericapositions = southamericapositions?.replace(/-/g, ' ')
  console.log(southamericapositions)
  console.log(southamericapositions)
  ...
  const { Name, Title, Description, Keyword, content: content1 } = southamericapositionsData.find(item => item.Name === southamericapositions);
  
  window.location.replace(`https://www.academicjobs.com/south-america/${southamericapositions?.replace(/\W+/g, '-').toLowerCase()}`);
  ...
  content = (
    <div className='flex-col gap-2'>
      ...
      <Link className='text-[#00aeef] mb-4 ' to='/South-America/'>← View all South America Jobs</Link>
      ...
      <SearchResults q={{ q: Name, l: "South-America" }} />
    </div>
  )
  return <div className='overflow-y w-full'>{content}</div>
}
export default SouthAmericaPositions
