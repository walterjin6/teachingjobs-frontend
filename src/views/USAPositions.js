...
import usapositionsData from "../utils/usapositions.json";
import { Helmet } from "react-helmet";
const USApositions = () => {  window.location.replace("https://www.academicjobs.com/usa");
  const dispatch = useDispatch()
  let { usapositions } = useParams();
  usapositions = usapositions?.replace(/-/g, ' ')
  console.log(usapositions)
  console.log(usapositions)
  ...
  content = (
    <div className='flex-col gap-2 container mx-auto'>
      ...
      <Link className='text-[#00aeef] mb-4 ' to='/USA/'>← View all USA Jobs</Link>
      ...
      <SearchResults q={{ q: Name, l: "USA" }} />
    </div>
  )
  return <div className='overflow-y w-full'>{content}</div>
}
export default USApositions
